...
> 📌 **Note for submission:** The 6 screenshots are saved in a `Screenshot/` folder next to this file, and they are already linked below. When you push to GitHub, keep the `Screenshot/` folder in the same place as `assignment2.md` so the images show up correctly.
...
- A second Reddit thread (Blaze_07 / fturriaf) also said the update removed useful information from the screen, showing this is not just a one-time complaint.
  ![Reddit comment — Blaze_07 / fturriaf](Screenshot/quickbooks-3-reddit-blaze07.png)
